  ...
  class PerseestClass extends Base {
    ...
    static async fetchMany (cond = {}) {
      ...
      switch (entries.length) {
        case 0:
          params = new Params({ conf: this.db, key: null, kval: null })
          break
        case 1:
          params = new Params({
            conf: this.db, key: entries[0][0], kval: entries[0][1]
          })
          break
        default:
          throw new Error('Multiple columns matches will hopefully be added soon')
      }
      try {
        return await this.db.queries.run('fetchMany', params)
      } catch (err) { throw err }
    }
    ...
    static async deleteMany (cond = {}) {
      ...
      switch (entries.length) {
        case 0:
          throw new Error('Attempted to call deleteMany without a delete condition')
        case 1:
          try {
        ...
        default:
          throw new Error('Hi, multiple columns matches will hopefully be added soon')
      }
    }
  }
