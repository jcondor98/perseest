...
class PerseestConfig {
   ...
   * @param {string} trigger - Action trigger for the hook
   * @param {HookParameters} params - parameters to pass to the hooks
   * @return undefined
   */
  async runHooks(when, trigger, params) {
    if (when && !['before', 'after'].includes(when))
      throw new TypeError(
        'Temporal trigger must be within [\'before\',\'after\']');
    if (trigger && !validate.isString(trigger))
      throw new TypeError('Action trigger must be a string if given');

    // TODO: Too many nested stuff, I don't like it at all
    for (const tm of when ? [when] : ['before', 'after']) {
      const triggers = trigger ? [trigger]
        : Object.getOwnPropertyNames(this.hooks[tm]);
      for (const act of triggers) {
        if (this.hooks[tm][act])
          for (const h of this.hooks[tm][act]) {
            try {
              const _ = h(params);
...
function Mixin(Base) {
  ...
  class PerseestClass extends Base {
    ...
    async save() {
      ...
      try {
        const hookParams = new HookParameters({ ent: this });
        await this.constructor.db.runHooks('before', 'save', hookParams);

        hookParams.res = await this.constructor.db.pool.query(
          this.constructor.db.queries.save(this));
        this.exists = true;

        await this.constructor.db.runHooks('after', 'save', hookParams);
      } catch (err) {
    ...
    async update(...args) {
      ...
      for (const k of keys) {
        ...
        if (!this.constructor.db.columns.has(k))
          throw new Error(`${k} is not present in the database table`);
      }

      // Query the database
      try {
        const hookParams = new HookParameters({ ent: this, columns: keys });
        await this.constructor.db.runHooks('before', 'update', hookParams);

        hookParams.res = await this.constructor.db.pool.query(
          this.constructor.db.queries.update(this, keys));

        await this.constructor.db.runHooks('after', 'update', hookParams);
      }
      catch (err) { throw err; }
    }
    ...
    static async fetch(key, value) {
      ...
      try {
        const hookParams = new HookParameters({ key: key, kval: value });
        await this.db.runHooks('before', 'fetch', hookParams);
        ...
        switch (response.rowCount) {
          ...
          case 1:
            ...
            break;
          default: throw new Error('Too many results were returned'); // TODO: ???
        }

        hookParams.res = response;
        hookParams.ent = ent;
        await this.db.runHooks('after', 'fetch', hookParams);
    ...
    static async delete(key, value) {
      ...
      try {
        const hookParams = new HookParameters({ key: key, kval: value });
        await this.db.runHooks('before', 'delete', hookParams);

        const response = await this.db.pool.query(
          this.db.queries.delete(key, value));

        // TODO: Move below to no be runned if multiple entities are removed?
        hookParams.res = response;
        await this.db.runHooks('after', 'delete', hookParams);
        ...
        switch (response.rowCount) {
          ...
          default: throw new Error(
            `${response.rowCount} entities were removed, expected 0 or 1`); // TODO: ???
        }
      }
      catch (err) { throw err; }
    }
...
// TODO: Find a way to extract primary key from entities
class HookParameters {
   ...
   *   however this could lead to weird behaviours if the hook tries to access
   *   them (e.g. both 'ent' and 'key' are null)
   * @param {object} opt - Arguments
   * @param {PerseestConfig} opt.conf - Perseest configuration for
   *   the entity class
   * @param {object} opt.res - postgres response (only in after-hooks)
   * @param {object} opt.ent - Persistent entity
   * @param {string} opt.key - Univocal identifier column name
   * @param {*} opt.kval - Value for 'key'
   * @param {Array<String>} opt.columns- Names of the columns to treat
   * @param {Array} opt.values - Values for 'columns', passed in order
   */
  constructor({ conf=null, res=null, ent=null, key=null, kval=null,
  ...
  get key() {
    return this.#_key || this.conf.primaryKey;
  }

  get kval() {
    return this.#_kval || this.ent[this.key];
  }

  get columns() {
    if (!this.#_columns) { // Column-value indexing must be consistent, init both
      this.#_columns = [...this.conf.columns];
